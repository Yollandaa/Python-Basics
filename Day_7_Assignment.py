...
class Bank:
    ...
    def withdraw(self, amount):
        ...
        self.add_transaction(
            len(self.transactions) + 1, datetime.now(), "withdraw", amount
        )
    ...
    def deposit(self, amount):
        self.balance += amount
        self.add_transaction(
            len(self.transactions) + 1, datetime.now(), "deposit", amount
        )
        return f"Success. Your balance is: R{self.balance:,}"

    def add_transaction(self, id, date, type, amount):
        transaction = {"id": id, "date": date, "type": type, "amount": amount}
    ...
    def print_statement(self):
        ...
        for transaction in self.transactions:
            print(
                f"{transaction['id']:<5} {transaction['date'].strftime('%d %b'):<10} {transaction['type']:<10} {transaction['amount']:>10}"
            )
